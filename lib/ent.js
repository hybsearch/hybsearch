...
function strictSearch(node, nmInstances=[], gnmlist=[]) {
	...
	if (node.branchset) {
		...
		let forRemoval = []
		for (let speciesSet of combinations) {
			// if species is in speciesList
			// continue
			let resultsA = strictSearch(speciesSet[1], nmInstances, gnmlist)
			let speciesListA = resultsA.species
			// nmInstances.push(...resultsA.nm)
			// nmInstances = nmInstances.concat(resultsA.nm)

			let resultsB = strictSearch(speciesSet[0], nmInstances, gnmlist)
			let speciesListB = resultsB.species
			// nmInstances.push(...resultsB.nm)
			// nmInstances = nmInstances.concat(resultsB.nm)
			...
			speciesListA.forEach(species1 => {
				...
				if (hasName && notAllEqual) {
					...
					speciesListA.forEach(species3 => {
						if (species3.name !== species1.name) {
							if(!nmInstances.filter(pair => isEqual(pair, [species1, species3])).length) {
							// if(!nmInstances.includes([species1,species3])){
								//console.log("nmInstances check: ",nmInstances)
								nmMark(node, species1, species3)
								// debug(`nmMark called on ${species1} and ${species3}`)
								//console.log(`nonmonophyly: ${label(species1)} / ${label(species3)}`)
								nmInstances.push([species1, species3])
								if (!some(gnmlist, pair => isEqual(pair, [species1, species3]))) {
					...
					speciesListB.forEach(species3 => {
						if (species3.name !== species1.name) {
							if(!nmInstances.filter(pair => isEqual(pair, [species1, species3])).length) {
								...
								forRemoval.push(species3.ident)
								//console.log(`removing from B ${label(species3)}`)
							}
						}
					})
				}
...
function formatData(results) {
	return gnmlist.map(pair => pair.map(label).join(' / ')).join('\n')
}
